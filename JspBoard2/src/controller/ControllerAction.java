...
public class ControllerAction extends HttpServlet {
  ...
  //Properties객체 생성
    ...
    private void requestPro(HttpServletRequest request,
    		                HttpServletResponse response) 
    		                			throws ServletException, IOException {
    	String view=null;//요청명령어에 따라서 이동할 페이지 저장->list.jsp
    	//요청명령어에 따른 요청명령어 클래스(=액션클래스)
    	/*
    	 * ListAction com=null; ListAction=new ListAction();
    	 * WriteFormAction com=null; WriteFormAction com=new WriteFormAction();
    	 * ,,,
    	 * */
    	//상속기법							//CommandAction com=new WriteFormAction();
    	CommandAction com=null;//CommandAction com=new ListAction();
    	//어떠한 자식클래스의 객체라도 부모형으로 형변환을 통해 받아올수 있다.
    		try {
    			//1.요청명령어를 분리->jsp결정
    			...
    			//요청명령어->/list.do->action,ListAction객체를 불러와서->requestPro()호출->/list.jsp
    			com=(CommandAction)commandMap.get(command);
    			System.out.println("com=>"+com);//action.ListAction@주소값
    			//요청명렁어에 따라서 어떠한 페이지로 이동할지 결정
    		...
    		//위에서 요청명렁어에 해당하는 view로 데이터를 공유시키면서 이동
    		RequestDispatcher dispatcher=request.getRequestDispatcher(view);// /list.jsp
    		dispatcher.forward(request, response);//데이터 공유하면서 특정jsp페이지로 이동하라.
    }
}
